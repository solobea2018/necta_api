...
// Construct NECTA URL
$url = "https://matokeo.necta.go.tz/results/{$year}/{$class}/index.htm";
if ($year>=2023 && $year<=2025){
    $url="https://onlinesys.necta.go.tz/results/{$year}/{$class}/index.htm";
}
...
foreach ($nodes as $node) {
    ...
    if (preg_match('/^([PS]\d{4})/i', $text, $match)) {
        ...
        $centres[] = [
            'school' => $schoolNo,
            'name'   => $text,
            'link'   => 'acsee_result.php?school=' . urlencode($schoolNo).'&year='.urlencode($year)
        ];
    }
}
